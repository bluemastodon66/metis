...
class RedisSessionStore:
    ...
    def prefixed(self, sid):
        return '%s:%s' % (self.options['session_prefix'], sid)
    def getSize(self):
        return str(self.redis.dbsize())
    ...
    def set_session(self, sid, session_data, name):
        expiry = self.options['session_timeout']
...
class Session:
    ...
    def __init__(self, session_store, sessionid=None):
        ...
        if sessionid is None:
            sessionid = self._store.generate_sid()
        #self.set_secure_cookie("session_id",sessionid)
        self._sessionid = sessionid
        self._sessiondata = self._store.get_session(self._sessionid, 'data')
        self.dirty = False

    def getCount(self):
        return self._store.getSize()
    def clear(self):
